...
static i_DES_cblock weak_keys[NUM_WEAK_KEY] =
{
	/* weak keys */
...
static const i_DES_LONG des_skb[8][64] =
{
	{
...
{
	return (i_DES_set_key(key, schedule));
}
...
int i_DES_set_key(i_const_DES_cblock *key, i_DES_key_schedule *schedule)
{
	if (i_DES_check_key)
	{
		return i_DES_set_key_checked(key, schedule);
	}
	else
	{
		i_DES_set_key_unchecked(key, schedule);
		return 0;
	}
}
...
int i_DES_is_weak_key(i_const_DES_cblock *key)
{
	...
	for (i=0; i<NUM_WEAK_KEY; i++)
		 ...
		 * bytes, 97/03/18 */
		if( memcmp(weak_keys[i], key, sizeof(i_DES_cblock)) == 0 ) return (1);
	return (0);
}
...
{
	if(!i_DES_check_key_parity(key))
		return (-1);
		
	if(i_DES_is_weak_key(key))
		return (-2);
...
void i_DES_set_key_unchecked(i_const_DES_cblock *key, i_DES_key_schedule *schedule)
{
	...
	register i_DES_LONG c, d, t, s, t2;
	register const unsigned char *in;
	register i_DES_LONG *k;
	register int i;
...
int i_DES_random_key(i_DES_cblock *ret)
{
	...
	{
		srand(seed);
		for(i=0;i<sizeof(i_DES_cblock);i++) ((unsigned char *)ret)[i] = (unsigned char)rand();
	}
